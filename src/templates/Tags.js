...
const Tags = ({ pageContext, data }) => {
  ...
  const tagHeader = `${totalCount} posts found with tag: ${tag}`
  return (
    <Layout>
      <Seo data={dataSeo} />
      ...
      {edges.map(({ node }) => {
        ...
        return (
          <Link to={'/' + slug} key={node.id}>
            <div className="transition duration-200 ease-in-out hover:bg-gray-800 hover:text-gray-100 rounded-md px-3 py-2 lg:mt-4 mt-2">
